...
/*
     fork from bitcoinjs-lib, make sign to async
 */
...
const transaction_1 = require('bitcoinjs-lib/src/transaction');
...
class TransactionBuilder {
    ...
    async async_sign(vin, keyPair, redeemScript, hashType, witnessValue, witnessScript) {
        // TODO: remove keyPair.network matching in 4.0.0
        if (keyPair.network && keyPair.network !== this.network)
            throw new TypeError('Inconsistent network');
        if (!this.__INPUTS[vin]) throw new Error('No input at index: ' + vin);
        ...
        // enforce in order signing of public keys
        const res = await Promise.all(input.pubkeys.map(async (pubKey, i) => {
            if (!ourPubKey.equals(pubKey)) return false;
            if (input.signatures[i]) throw new Error('Signature already exists');
            // TODO: add tests
            if (ourPubKey.length !== 33 && input.hasWitness) {
            ...
            const signature = await keyPair.sign(signatureHash, this.__USE_LOW_R);
            input.signatures[i] = bscript.signature.encode(signature, hashType);
            return true;
        }));
        const signed = res.some(i=>i);
        if (!signed) throw new Error('Key pair cannot sign for this input');
    }
