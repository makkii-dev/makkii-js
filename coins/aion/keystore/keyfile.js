...
function fromV3(input, password){
    ...
    if(KeystoreItem.length === 4){
        ...
        const keyStoreContents = RLP.decode(input);
        let keyStoreContent = [];
        for (let i = 0; i< keyStoreContents.length; i++){
        ...
        if (keyStoreContent[5] === 'scrypt'){
            derivedKey = scrypt(
                Buffer.from(password),
                Buffer.from(keyStoreContent[7], 'hex'),
                parseInt(keyStoreContent[8],16),
                parseInt(keyStoreContent[9],16),
                parseInt(keyStoreContent[10],16),
                parseInt(keyStoreContent[6],16)
            )
        }else {
            throw new Error('Unsupported key derivation scheme')
        }

        const ciphertext = Buffer.from(keyStoreContent[2], 'hex')
        const actual = blake2b(32).update(Buffer.concat([derivedKey.slice(16,32),ciphertext])).digest('hex');
        if(actual!== keyStoreContent[11].toString('hex')){
            throw new Error("Invalid Password!");
        }
        ...
        const decipher = crypto.createDecipheriv(
            keyStoreContent[4],
            derivedKey.slice(0, 16),
            Buffer.from(keyStoreContent[3], 'hex')
        );
        const seed = runCipherBuffer(decipher, ciphertext);
        return keyPair(seed);
    }
