...
const fetchAccountTokenBalance = (contractAddress, address, network) =>
    new Promise((resolve, reject) => {
        ...
        console.log('[ETH get token balance req]:', config.networks[network].jsonrpc);
...
const fetchTokenDetail = (contractAddress, network) =>
    new Promise((resolve, reject) => {
        ...
        console.log('[ETH get token detail req]:', config.networks[network].jsonrpc);
        axios
...
const fetchAccountTokenTransferHistory = (address, symbolAddress, network, page = 0, size = 25, timestamp) =>
    new Promise((resolve, reject) => {
        const {explorer_api} = config.networks[network];
        if (explorer_api.provider === "etherscan") {
            const url = `${explorer_api.url}?module=account&action=tokentx&contractaddress=${symbolAddress}&address=${address}&page=${page}&offset=${size}&sort=asc&apikey=${config.etherscanApikey}`;
            ...
            const url = `${explorer_api.url}/getAddressHistory/${address}?apiKey=${config.ethplorerApiKey}&token=${symbolAddress}&type=transfer&limit=${size}&timestamp=${timestamp / 1000 - 1}`;
            console.log(`[eth http req] get token history by address: ${url}`);
            HttpClient.get(url)
                .then(res => {
                    const transfers = {};
...
function getTopTokens(topN = 20, network) {
    return new Promise((resolve, reject) => {
        // const url = `${config.networks[network].remote}/token/eth/search?offset=0&limit=${topN}`;
