...
const getBalance = (address, network = 'mainnet') =>
    new Promise((resolve, reject) => {
        const url = `${config.networks[network].jsonrpc}/wallet/getaccount`;
...
const getLatestBlock = (network = 'mainnet') =>
    new Promise(resolve => {
        const url = `${config.networks[network].jsonrpc}/wallet/getnowblock`;
...
const broadcastTransaction = (tx, network = 'mainnet') =>
    new Promise(resolve => {
        const url = `${config.networks[network].jsonrpc}/wallet/broadcasttransaction`;
...
const getTransactionById = (hash, network = 'mainnet') =>
    new Promise(resolve => {
        const url = `${config.networks[network].jsonrpc}/walletsolidity/gettransactionbyid`;
...
const getTransactionInfoById = (hash, network = 'mainnet') =>
    new Promise(resolve => {
        const url = `${config.networks[network].jsonrpc}/walletsolidity/gettransactioninfobyid`;
        const promise = HttpClient.post(
            url,
            {
                value: hash,
