...
var fromV3 = function (input, password) { return new Promise(function (resolve, reject) {
    try {
        ...
        var actual = blake2b(32).update(Buffer.concat([derivedKey.slice(16, 32), ciphertext])).digest('hex');
...
function toV3(privateKey, password) {
    ...
    var mac = blake2b(32).update(Buffer.concat([Buffer.from(derivedKey.slice(16, 32)), ciphertext])).digest();
    var crypto_ = [];
    ...
    var keystore = [];
    keystore[0] = utils_1.crypto.randomBytes(4).toString('hex') + "-" + utils_1.crypto.randomBytes(2).toString('hex') + "-" + utils_1.crypto.randomBytes(2).toString('hex') + "-" + utils_1.crypto.randomBytes(2).toString('hex') + "-" + utils_1.crypto.randomBytes(6).toString('hex');
    keystore[1] = 3;
