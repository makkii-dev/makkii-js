...
function getTransactionsByAddress(address, page = 0, size = 25, network = 'mainnet') {
  const url = `${config.networks[network].explorer_api}/aion/dashboard/getTransactionsByAddress?accountAddress=${address.toLowerCase()}&page=${page}&size=${size}`;
...
function getTransactionUrlInExplorer(txHash, network = 'mainnet') {
  return `${config.networks[network].explorer}/${txHash}`;
}
