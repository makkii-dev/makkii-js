...
export default (config) => {
  ...
  const getAccountTokenBalance = async (contractAddress, address) => {
    ...
    if (res.data.result) {
      return (new BigNumber(AbiCoder.decodeParameter('uint128', res.data.result)));
    } else {
  ...
  const getTokenDetail = async (contractAddress) => {
    ...
    const [symbolRet, nameRet, decimalsRet] = await Promise.all([promiseSymbol, promiseName, promiseDecimals]);
  ...
  async function getAccountTokenTransferHistory(address, symbolAddress, page = 0, size = 25) {
