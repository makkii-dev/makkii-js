...
exports.signTransaction = (transaction, network = 'BTC') => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (extra_param && extra_param.type === '[ledger]') {
        ...
        const { address, publicKey } = yield ledger_1.getKeyByLedger(derivationIndex, network);
        if (sender !== address) {
        ...
        const paths = [];
        for (let ip = 0; ip < utxos.length; ip += 1) {
            const preTxhex = yield jsonrpc_1.getRawTx(utxos[ip].hash, network);
            const preTx = ledger_1.wallet.splitTransaction(preTxhex);
            inputs.push([
                preTx,
                utxos[ip].index,
            ]);
