...
const getBalance = async (address, network = 'BTC') => {
  const url = `${config.networks[network].jsonrpc}/addr/${address}`;
...
const getUnspentTx = async (address, network = 'BTC') => {
  const url = `${config.networks[network].jsonrpc}/addr/${address}/utxo`;
...
const getRawTx = async (txhash, network = 'BTC') => {
  const url = `${config.networks[network].jsonrpc}/rawtx/${txhash}`;
...
const broadcastTransaction = async (encoded, network) => {
  const url = `${config.networks[network].broadcast}`;
...
const getTransactionStatus = async (txId, network = 'BTC') => {
  const url = `${config.networks[network].jsonrpc}/tx/${txId}`;
...
const getTransactionsByAddress = async (address, page, size, timestamp = undefined, network = 'BTC') => {
  const url = `${config.networks[network].jsonrpc}/txs/?address=${address}&pageNum=${page}`;
